...
(function (EP) {
  ...
  /** The option tenors the wheel offers on both legs. Only these are liquid
   *  enough in real markets to be worth modelling, which is why they are a
   *  dropdown rather than a slider. */
  const TENOR_OPTIONS = [
    { value: "1", label: "1 month" }, { value: "3", label: "3 months" },
    { value: "6", label: "6 months" }, { value: "9", label: "9 months" },
    { value: "12", label: "12 months" },
  ];
  const tenorFmt = (v) => `${v} month${String(v) === "1" ? "" : "s"}`;

  /** Fraction of days the wheel actually held stock, replayed off the event
   *  tape. This is the number that decides the strategy's return -- two
   *  earlier rule sets each looked reasonable and each drove it near zero --
   *  so it belongs on a tile rather than buried in the prose. Share counts,
   *  not a boolean: a partial call-away leaves the account still long. */
  function heldFraction(arm) {
    const n = arm.equity.length - 1;
    if (n <= 0) return 0;
    const byDay = new Map();
    for (const e of arm.events) {
      if (!byDay.has(e.t)) byDay.set(e.t, []);
      byDay.get(e.t).push(e);
    }
    let shares = 0, longDays = 0;
    for (let t = 1; t <= n; t++) {
      for (const e of byDay.get(t) || []) {
        if (e.kind === "assigned") shares += e.contracts * 100;
        else if (e.kind === "buy_shares") shares += e.contracts;
        else if (e.kind === "called_away") shares -= e.contracts * 100;
        else if (e.kind === "stop_shares") shares = 0;
      }
      if (shares > 0) longDays++;
    }
    return longDays / n;
  }
  ...
  const SCENARIOS = [
    ...
    },
    {
      ...
      status: "ready",
      why:
        "Tests whether a popular options income strategy actually beats " +
        "simply owning the asset.",
      name: "The wheel strategy",
      blurb:
        "Never hold idle cash: sell puts with it, take the shares when " +
        "assigned, and only sell calls at record highs — against the stock " +
        "alone, and against doing nothing at all.",
      story:
        ...
        "Traders call this cycle “the wheel,” and it looks like " +
        "manufacturing income from nothing. Selling puts and calls is " +
        "decades old; running them as one loop is newer, popularised on " +
        "trading forums in the 2010s. Whether it beats simply holding the " +
        "stock depends far less on the premium than on how much of the " +
        "time you own anything at all.",
      controls: [
        ...
        // The wheel itself no longer reads this: it writes calls at *record*
        // highs, which needs no tolerance band. It still drives the
        // buy-the-dip comparison arm, so the label says whose dial it is.
        { key: "dipPct", label: "Dip trigger (buy-the-dip arm only)", min: 0.01,
          max: 0.15, step: 0.01, value: 0.05, fmt: pct },
        // Both tenors, in months. Selects rather than sliders because only
        // these are liquid enough to be realistic. They are separate dials
        // because the two legs are not symmetric: a longer put collects more
        // premium per contract but commits the collateral for longer before
        // it can be assigned, while a longer call sells away more of the
        // upside per contract but is written far less often.
        { key: "xMonths", type: "select", label: "Cash-secured put length",
          value: "6", options: TENOR_OPTIONS, fmt: tenorFmt },
        { key: "yMonths", type: "select", label: "Covered-call length",
          value: "3", options: TENOR_OPTIONS, fmt: tenorFmt },
      ...
      fixed: { s0: 100, r: 0.03, q: 0, sellHaircut: 0.10,
      ...
      derive: (pr) => {
        // A <select> hands back a string; both tenors are arithmetic
        // downstream. xMonths additionally sets the buy-the-dip arm's
        // rolling-high lookback and the horizon of the exact single-put
        // assignment-probability tile, so those track the put you are
        // actually selling rather than a hardcoded six months.
        pr.xMonths = Number(pr.xMonths);
      ...
      compute: (pr) => {
        ...
        return {
          stats: Object.assign({
            ...
            holdFraction: heldFraction(fam.wheel),
          }, w),
          fam, sweep,
        };
      },
      charts: ["wheel-paths", "wheel-bars", "wheel-sweep"],
      // CONDITIONAL: the deciding quantities are the vol spread
      // (pr.sigmaIv - pr.sigmaRv, net of the 10% sale haircut and the per-
      // contract fees) and s.holdFraction, the share of days actually long the
      // stock. The verdict is settled by whether s.wheelCagr clears
      // s.holdCagrExact.
      verdict: (pr, s) => {
        const spread = pr.sigmaIv - pr.sigmaRv;
        const edge = s.wheelCagr - s.holdCagrExact;
        const shared =
          `The wheel compounds ${pctSigned(s.wheelCagr)} a year against buy-and-hold's ` +
          `${pctSigned(s.holdCagrExact)}, while long the stock ${pct(s.holdFraction)} of the days — that ` +
          `exposure, not the premium, is most of the return. You sold ${count(s.putsSold)} puts at a ` +
          `${pctSigned(spread)} vol spread, before the ${pct(pr.sellHaircut)} sale haircut and fees.`;
        if (spread <= 0) {
          return {
            tone: "bad",
            headline: "You are paid less volatility than the stock delivers — this cannot pay, so just hold it.",
            body:
              shared +
              ` With implied vol at ${pct(pr.sigmaIv)} below realized at ${pct(pr.sigmaRv)}, the seller is ` +
              `systematically underpaid for the risk taken; no amount of loop discipline recovers a negative edge.`,
          };
        }
        if (edge <= 0) {
          return {
            tone: "bad",
            headline: "Even with a positive vol spread the wheel trails buy-and-hold — hold the stock instead.",
            body:
              shared +
              ` The ${pctSigned(spread)} spread does not cover the haircut and the fees, and the covered ` +
              `calls cap exactly the upside you are paid to give away — a gap of ${pctSigned(edge)} a year.`,
          };
        }
        return {
          tone: "good",
          headline: `The wheel is ${pctSigned(edge)} a year ahead of holding — but the vol spread pays, not the loop.`,
          body:
            shared +
            ` Watch the sweep: shrink the spread toward zero and the advantage goes with it — a bet on ` +
            `implied volatility exceeding realized by more than costs, dressed up as income.`,
        };
      },
      tiles: (pr, s) => [
        ...
        { label: "Time actually holding the stock",
          value: pct(s.holdFraction),
          note: "the number that decides everything — see the note below" },
        { label: "A single put's real-world odds of assignment",
          value: pct(s.putNaiveAssignProb),
          note: `exact; the run assigned ${pct(s.simAssignRate)} of ${count(s.putsSold)} puts sold` },
      ],
      mathBox: (pr, s) => [
        "No closed form for the account overall, honestly: entry timing, the",
        "assignment cycle and the share stop are all path-dependent. What IS",
        "exact is one option's own real-world assignment odds, under the real",
        "measure (drift \\(\\mu\\)), not the risk-neutral one:",
        `\\(P(\\text{assigned}) = \\Phi\\!\\left(\\dfrac{\\ln(K/S) - (\\mu - q - \\sigma^2/2)T}{\\sigma\\sqrt{T}}\\right)\\) = ${pct(s.putNaiveAssignProb)}`,
        `A cash-secured put breaks even at \\(K - \\text{premium}\\); the wheel's`,
        `edge is \\(\\sigma_{IV} - \\sigma_{RV}\\) net of costs — the sweep below.`,
      ],
      notation: {
        "\\(S\\)": "Current stock price",
        "\\(K\\)": "Option strike price (exercise price)",
        "\\(T\\)": "Time to expiry, in years",
        "\\(\\mu\\)": "The stock's real-world drift (not the risk-free rate)",
        "\\(\\sigma_{IV}\\)": "Implied volatility — what you are paid to sell options at",
        "\\(\\sigma_{RV}\\)": "Realized volatility — what the stock actually delivers",
        "\\(\\Phi\\)": "The standard normal CDF",
        "premium": "What you receive for selling the option",
        "basis": "Share-weighted average cost of the shares held",
      },
      note:
        "Return here is decided almost entirely by one number that has " +
        "nothing to do with options: how much of the time you actually own " +
        "the stock. Two earlier rule sets drove it near zero, and both read " +
        "as ordinary discipline. Waiting for a 5% dip before selling each new " +
        "put left the account in cash 95% of the S&P's 2009-2026 history — " +
        "but the put is what pays you to wait. And writing the call as soon " +
        "as the shares arrived struck every one at the cost basis, since " +
        "assignment happens precisely when price sits below the strike that " +
        "bought the shares. What runs now: idle cash always sells a put, held " +
        "or not; cash too small for a contract buys stock outright; calls are " +
        "written only at a record high, which is by construction above every " +
        "price that ever bought shares. The one loss cap sits on the shares — " +
        "the same risk buy-and-hold runs. The sweep generalises past this " +
        "path. It asks the only question that decides whether selling options " +
        "works: does implied volatility exceed realized by more than the " +
        "haircut and fees? Below that line the wheel is not yield, just an " +
        "expensive way to hold stock.",
    },
    {
      id: "base-rates",
      status: "ready",
