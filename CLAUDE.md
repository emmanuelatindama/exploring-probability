...
| Path | Role |
|---|---|
| `lab/analytics.py` | **Source of truth.** Closed-form expectations, quantiles, tail probabilities, Kelly optimum. |
| `lab/verify.py` | Checks the closed forms against Monte Carlo, then writes `js/golden.js`. |
| `js/engine.js` | Simulation + the same closed forms, mirroring `analytics.py`. |
...
| `js/app.js` | Page wiring: controls, tiles, legends, table views, theme. |
...
`tests.html` asserts bit-for-bit equality. Do not "improve" one side alone.
...
- Palette is the skill's reference instance, in CSS custom properties in
  `index.html`. Series slots 1–3 (blue/orange/aqua) validate all-pairs in both
  modes; the sweep chart uses the blue↔red diverging pair around a zero baseline.
